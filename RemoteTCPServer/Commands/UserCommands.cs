...
namespace RemoteTCPServer.Commands
{
    ...
    public class UserCommands : IClientCommand
    {
        ...
        private List<Cmd> _commands = CommandFactory.GetUser();
        ...
        public string Call(string data)
        {
            ...
            if (function != null && function.Access >= ClientOwner.CUser.GetAccess())
                return function.Execute(subCommands);
        ...
        internal string GetAllClients( string[] args = null)
        {
            ...
            {
                clientList += GetClientInfo();
                clientPos++;
            }
            return clientList;
        }
        internal string KickClient( string[] strPos)
        {
            if (!ClientOwner.CUser.AllowedAccess(0)) return "Access Level not high enough.";
        ...
        internal string RestartServer( string[] confirmed)
        {
        ...
        internal string GetClientInfo(string[] args = null)
        {
            string id = "Not logged in";
            if (ClientOwner.CUser != null) id = ClientOwner.CUser.ID;

            return $"\n\nID: {id}\n" +
                $"IP address: {ClientOwner.IP}\n" +
                $"Machine name: {ClientOwner.MachineName}\n" +
                $"Connection order: {SP.Clients.IndexOf(ClientOwner)}\n" +
                $"Handle: {ClientOwner.CSocket.Handle}\n";
        }
        ...
        internal string Messaging( string[] fileData)
        {
            ...
            string help = "Message other active clients\n" +
                ...
                $"   e.g. {fileData[0]} message {ClientOwner.IP}:{ClientOwner.CSocket.Handle.ToString()} Hello there!\n\n" +
        ...
        internal string UserSecurity( string[] fileData)
        {
        ...
        internal string SqlCommands(string[] input)
        {
            return ClientOwner.SqlCommands.Call(string.Join(' ',input));
        }
    }
}
